...
public class Config {
    ...
    public static void setup() { // NO_UCD (use default)
        file = new File(
                Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("Glow")).getDataFolder(),
                "config.yml");
        if (!file.exists()) {
    ...
    public static void fileExists() { // NO_UCD (use private)
        try {
            @SuppressWarnings("unused") final String filesString = Arrays
                    .toString(Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("Glow"))
